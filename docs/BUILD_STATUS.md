...
- Open blocker: evidence/report hash-pinned `final` revision은 아직 배포되지 않았다. 9-page deck은 demo5 값으로 재export·렌더 QA를 통과했지만, QA-failed provisional video는 제거됐고 final file/URL은 아직 없다.
...
| Local repository gates | implemented | local | local | unverified | — | `package.json`; `artifacts/local/firestore-emulator-verification.json` | P0 | build/lint/typecheck/full Vitest, emulator `10/10`, template/diff/boundary passed. The last `75/0/0` submission run included the now-removed QA-failed video, so current-tree submission verification remains open until a final video exists |
